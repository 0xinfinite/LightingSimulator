...
#include "../HDRP_old/Environment/AbleEnviornmentLighting.hlsl"
...
half3 AbleTerrainLighting(BRDFData brdfData, BRDFData brdfDataClearCoat,
    half3 lightColor, half3 lightDirectionWS, half lightAttenuation,
    half3 normalWS, half3 viewDirectionWS,
    half clearCoatMask, bool specularHighlightsOff,
    half shadowFactor = 0, half setLinear = 0,
    half shadowPowFactor = 1,
half3 shadowColor = 0, half3 shadowBoundaryColor = 0)
{
    ...
    float3 radiance = lightColor * finalShadowValue;
...
half3 AbleTerrainLighting(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, half3 normalWS, half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff,
    half shadowFactor = 0, half setLinear = 0,
    half shadowPowFactor = 0,
half3 shadowColor = 0, half3 shadowBoundaryColor = 0)
{
    return AbleTerrainLighting(brdfData, brdfDataClearCoat, light.color, light.direction, light.distanceAttenuation * light.shadowAttenuation, normalWS, viewDirectionWS, clearCoatMask, specularHighlightsOff,
    shadowFactor, setLinear, shadowPowFactor, shadowColor, shadowBoundaryColor);

}

half4 AbleTerrainPBR(InputData inputData, SurfaceData surfaceData, BRDFData brdfData, float2 uv, float3 positionWS, half setLinear, half shadowPowFactor,
...
//    aoFactor.indirectAmbientOcclusion -= lerp(0, /*_AOPowerFactor2*/1, addAOFactor);
//    uint meshRenderingLayers = GetMeshRenderingLightLayer();
//    Light mainLight = GetMainLight(inputData, shadowMask, aoFactor);
//    //half addShadowFactor = step(mainLight.shadowAttenuation, 0.8);
//    //mainLight.shadowAttenuation  -= lerp(0, /*_ShadowPowerFactor2*/1, addShadowFactor);//= pow(mainLight.shadowAttenuation, _ShadowPowerFactor);
...
//                                                                                    /*_ShadowPowerFactor2*/0, setLinear,
...
//    return CalculateEnvLitCoreFinalColor(lightingData, surfaceData.alpha);
    ...
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
    {
        lightingData.mainLightColor = AbleTerrainLighting(brdfData, brdfDataClearCoat,
                                                              mainLight,
                                                              inputData.normalWS, inputData.viewDirectionWS,
                                                              surfaceData.clearCoatMask, specularHighlightsOff, 0, 0, 1, 
                                                                _ShadowColor, _ShadowBoundaryColor);
    }
    ...
    for (uint lightIndex = 0; lightIndex < min(_AdditionalLightsDirectionalCount, MAX_VISIBLE_LIGHTS); lightIndex++)
    {
        ...
        if (IsMatchingLightLayer(light.layerMask, meshRenderingLayers))
        {
            lightingData.additionalLightsColor += AbleTerrainLighting(brdfData, brdfDataClearCoat, light,
                                                                          inputData.normalWS, inputData.viewDirectionWS,
                                                                          surfaceData.clearCoatMask, specularHighlightsOff, 0, 0, 1, 
                                                                _ShadowColor, _ShadowBoundaryColor);
        }
...
#endif
{
    ...
    AbleTerrainPBR(inputData, surfaceData, brdfData, IN.uvMainAndLM.xy, IN.positionWS, /*_ShadowPowerUseLinear*/1, _ShadowPowerPowFactor, _ShadowColor, _ShadowBoundaryColor);
